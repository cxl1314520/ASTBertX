...
def load_jsonl(path):
    ...
    with open(path, 'r', encoding='utf-8') as f:
        for line in f:
            try:
                ...
                filenames.append(item.get("filename", "unknown"))  # 保存文件名
    ...
    features, raw_labels, filenames = zip(*sorted_data)
    ...
    return np.array(features), np.array(labels), label_map, filenames
...
def train():
    ...
    # 加载数据 - 现在返回4个值，但我们只需要前3个
    features, labels, label_map, _ = load_jsonl(Config.DATA_PATH)  # 修改：接收4个值，忽略文件名
    ...
    print("\n🔍 验证模型保存一致性...")
    # 重新加载模型进行验证
    verification_model = MLP(input_dim=input_dim, num_classes=num_classes).to(device)
    verification_model.load_state_dict(torch.load(Config.BERT_MODEL_PATH))
    verification_model.eval()
    
    # 使用相同的验证集验证
    verification_loss, verification_acc = evaluate(verification_model, val_loader, criterion, device)
    print(f"✅ 验证集重新加载准确率: {verification_acc:.4f}")
    
    # 确保准确率一致
    if abs(verification_acc - val_acc) < 0.001:
        print("✅ 模型保存和加载一致性验证通过")
    else:
        print(f"⚠️ 模型保存和加载存在差异: 原准确率={val_acc:.4f}, 重新加载准确率={verification_acc:.4f}")
